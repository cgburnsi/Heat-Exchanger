...
class TubeBankZone(BaseZone):
    ...
    def build_geometry(self):
        ...
        n_even, _ = self._rows_in_column(even_offset)
        n_odd, _ = self._rows_in_column(odd_offset)
        self.n_rows_avg = (n_even + n_odd) / 2.0 
        
        centers = []
        for j in range(self.n_cols):
            col_x = self.origin_x + j * self.S_L
            centers.append((col_x, 0))
        self.tube_centers = centers
        self.n_tubes = len(centers) * self.n_rows_avg
        return centers
    ...
    def solve(self, hot_state_in, cold_state_in):
        ...
        str_g, str_c = hot_state_in.fluid_string, cold_state_in.fluid_string
        ...
        for i in range(self.n_cols):
            ...
            model_params = {
                'eps_por': self.eps_por, 'T': Tg, 'rho': rho_g, 'mu': mu_g, 'M_gas': M_g,
